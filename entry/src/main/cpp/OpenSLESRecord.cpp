...
#include "jni.h"
#define LOGE(... )   ((void)HiLogPrint((LOG_APP), LOG_ERROR, LOG_DOMAIN, LOG_TAG, __VA_ARGS__))

SLObjectItf engineObject;
SLEngineItf audioEngine;


static void RecordCallback(SLBufferQueueItf bufferQueue, void *context) {
...
static void PlayCallBack(SLBufferQueueItf pcmBufferQueue, void * context)
{
    OpenSLESRecord *recorder = (OpenSLESRecord*) context;
...
SLBufferQueueItf recorderBufferQueue; //Buffer接口
...
void OpenSLESRecord::createRecorder(){
    ...
    SLDataLocator_BufferQueue buffer_queue = {
            SL_DATALOCATOR_BUFFERQUEUE,    //类型 这里只能是这个常量
            2                                           //buffer的数量
    };
    ...
    // 获取Buffer接口
    (*recorderObject)->GetInterface(recorderObject, SL_IID_BUFFERQUEUE,
                                    &recorderBufferQueue);
...
void OpenSLESRecord::startRecord(const char * path) {
...
SLBufferQueueItf simpleBufferQueueItf;
